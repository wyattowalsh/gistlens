...
- Original Vite/React components (preserved in `src/` for reference)
- Client-side localStorage for settings (migrated to PostgreSQL)
- Manual token entry UI (replaced with OAuth)

#### Migration Path
All v1.0 features will be migrated to v2.0 architecture:
- UI components from `src/components/` → `components/`
- Client-side features → Server/Client Component split
